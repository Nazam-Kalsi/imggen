...
function Page({}: Props) {
  ...
  const fetchUserImages = async () => {
    ...
    if (res.res.data.data.data.images[0].isGeneratedSuccessfully) setPolling(false);
    ...
    <div>
      ...
      <div className="flex w-full flex-col gap-2 items-center justify-center p-4">
        ...
        <div className="flex justify-center items-center gap-4 flex-wrap min-h-[80vh] border w-full rounded-md">
          {userImages &&
            userImages.map((x, index) => 
              <ImageCard key={index} imageUrl={x.imageUrl} description={x.prompt} fileName="sample-image.jpg" />
            )}
        </div>
      </div>
      <Pagination>
